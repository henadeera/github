...
abstract class Farm_CPU {
  ...
  public List<Object> main() throws InterruptedException, ExecutionException {
    ...
    List<int[]> splittedArrays = split();
    for(int i = 0; i < splittedArrays.size(); i++) {
      final int[] srcObjArray = splittedArrays.get(i);
  ...
  public Object callExecuter(int[] srcObjArray) {
    CpuJobsExecuter cpuJobsExecuter = new CpuJobsExecuter();
    return cpuJobsExecuter.execute(srcObjArray);
  }
}